...
public class ClientUtilities {
    public static void registerItemProperties() {
        ClampedItemPropertyFunction predicate = (stack, level, player, seed) -> {
            return ModGunItem.getCharged(stack) ? 1 : 0;
        };
        ResourceLocation location = Treasures_of_the_dead.resource("loaded");
        ItemProperties.register(ModItems.PISTOL.get(), location, predicate);
    }

    public static Optional<HumanoidModel.ArmPose> getArmPose(Player player, InteractionHand hand) {
        ItemStack stack = player.getItemInHand(hand);
        if (!player.swinging && !stack.isEmpty() && stack.getItem() instanceof ModGunItem gun) {
            if (gun.canUseFrom(player, hand) && ModGunItem.getCharged(stack)) {
...
//        }
        ...
        if (swingProgress > 0) {
            ...
            if (player.getTicksUsingItem() <= gun.getUseDuration(stack, player)) {
...
//            else if (player.getTicksUsingItem() % 6 == 0) {
//                poseStack.translate(0, 0.1, 0);
//            }
//            Pair<Integer, Integer> loadingDuration = ModGunItem.getLoadingDuration(stack);
...
//            int loadingStage = ModGunItem.getLoadingStage(stack) + (int)(usingTicks / ticksPerLoadingStage);
//            int reloadDuration = ModGunItem.reloadDuration(stack);
//
...
//
//                if (false) { //gun == Items.BLUNDERBUSS
//                    poseStack.translate(0, 0, -0.06);
//                } else if (gun == ModItems.PISTOL.get()) {
